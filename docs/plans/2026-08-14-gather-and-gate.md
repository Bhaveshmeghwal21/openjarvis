...
**Amended post-implementation** (final whole-branch adversarial review, Finding 2d/7a, with a residual regression caught by an independent re-review and corrected in a second pass):
the reference code above specifies `def calibrate(signal_rows, labels, target_recall=GATE_RECALL_TARGET, floor=0.0)`. A default floor of `0.0` is the exact degenerate value the `floor` parameter's own docstring says it exists to prevent: if any single signal has zero variance among labeled-relevant papers (a realistic case — e.g. `graph` is `0.0` for every relevant paper before any citation expansion has produced a hit), `calibrate()` sets that signal's threshold to `max(0.0, 0.0) = 0.0`. Since `decide()`'s comparison is inclusive (`>=`), a threshold of exactly `0.0` is cleared by *every* candidate scoring `0.0` on that signal too — which is most of the gather set for a fresh project — so the gate silently degenerates into "keep everything," the opposite failure from what the union design protects against, but no less a defeat of the gate's purpose.

The first fix attempt simply raised the default floor to a flat constant (`MIN_CALIBRATED_THRESHOLD = 0.05`, applied via `max(floor, scores[index])`). An independent re-review caught that this traded one failure for another: a signal with *real* fine-grained separation clustered strictly below `0.05` (small but genuinely discriminating scores, not degenerate zeros) would have its threshold clamped up to `0.05` regardless, dropping recall on that signal below `target_recall` for no benefit — confirmed by construction (a seed where relevant papers score `0.01`–`0.048` on one signal: recall fell from 1.0 to 0.5 under the flat-floor fix).

The corrected design distinguishes *why* the floor exists from *when* it should apply: `floor` becomes `float | None = None`. Left at its default, the floor (`MIN_CALIBRATED_THRESHOLD`) is applied only to a signal whose raw quantile fit is exactly `0.0` — i.e., only to signals that are genuinely degenerate, never to one with any real separation, however small. Passed explicitly (any value, including `0.0`), the floor instead behaves as the original unconditional minimum — the caller has stated a stronger intent than "rescue dead signals only," and every signal is clamped up to at least that value regardless of its raw fit. This preserves the plan's own original Task 9 test (`test_calibration_respects_a_floor_so_a_signal_never_admits_everything`, which passes an explicit `floor=0.2` against a deliberately-nonzero raw fit and expects it to win) while fixing both the original degenerate-signal bug and the flat-floor regression. See `test_calibration_never_produces_a_threshold_that_admits_a_zero_scoring_signal`, `test_the_default_floor_does_not_sacrifice_recall_on_a_signal_with_real_separation`, and `test_an_explicit_floor_is_unconditional_even_when_the_raw_fit_is_nonzero` in `tests/test_gate.py`.
